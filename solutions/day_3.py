...
for i in range(len(myInput)):
    numLocs = [
        (j.start(), j.end()) for j in re.finditer(r"\d+", myInput[i])
    ]  # Find all the nums in the line
    for loc in numLocs:
        for pos, x, y in itertools.product(
            range(loc[0], loc[1]), range(-1, 2), range(-1, 2)
        ):
            ...
            if (
                rowBounds
                and colBounds
                and re.match("[^(\d|\.)]", myInput[i + x][pos + y])
            ):
...
for i in range(len(myInput)):
    ...
    starLocs = [
        star.start() for star in re.finditer(r"\*", myInput[i])
    ]  # Asterisk positions in the current line
    for loc in starLocs:
        nums = []  # Keep track of adjacent numbers
        if (
            i > 0
        ):  # If this isn't the first line, check the adjacent spaces in the previous line
            above = myInput[i - 1][max(0, loc - 1) : min(lineLen, loc + 2)]
            matches = re.finditer(r"\d+", above)
            nums += [
                expandNum(
                    myInput[i - 1], match.start() + loc - 1, match.end() + loc - 1
                )
                for match in matches
            ]
        if (
            i < len(myInput) - 1
        ):  # If this isn't the last line, check the adjacent spaces in the next line
        ...
        if myInput[i][min(lineLen - 1, loc + 1)].isdigit():  # Check to the right
            nums.append(
                expandNum(
                    myInput[i], min(lineLen - 1, loc + 1), min(lineLen - 1, loc + 1)
                )
            )
        if len(nums) == 2:
