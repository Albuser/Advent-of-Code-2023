...
def parseLine(line, mapMap):
    key, left, right = [re.search(regex, line).group(1) for regex in [r'^(\w+)', r'\((\w+)', r'(\w+)\)']]
    mapMap[key] = (left, right)

mapMap = {}
for line in myInput[2:]: parseLine(line, mapMap)
...
while not all([loc[-1] == 'Z' for loc in curLocs]):
    ...
    for loc in pending:
        curLocs[loc[0]] = mapMap[loc[1]][directions[startSteps[loc[0]] % len(directions)]]
        startSteps[loc[0]] += 1
